...
import { AnimateOnScroll, LuxuryCardTilt } from "@/components/AnimateOnScroll";
...
const ALL_CATALOG_ITEMS: CatalogItem[] = [
  ...
  },
  {
    ...
    category: "HAUTE COUTURE",
    name: "AURA NOCTURNE",
    subtitleEN: "Midnight Navy Tulle & French Lace Corset Gown",
    subtitleUA: "Нічна Сукня з Нічного Тюлю та Французького Мережива",
    descriptionEN:
      "A statement evening masterpiece featuring an architectural hand-embroidered French lace corset bodice, hand-sewn crystal beadwork, a multi-layered cascading midnight navy silk tulle skirt, and a daring high leg slit.",
    descriptionUA:
    ...
    priceEN: "Upon Request",
    priceUA: "За Запитом",
    silhouetteEN: "Ballgown with High Slit",
    silhouetteUA: "Кутюрна Бальна Сукня з Розрізом",
    fabricEN: "100% Silk Tulle & French Chantilly Lace",
    fabricUA: "100% Шовковий Тюль та Мереживо Шантильї",
    fittingTimeEN: "14-21 Business Days",
    fittingTimeUA: "14-21 Робочих Днів",
  ...
  },
  {
    ...
    subtitleEN: "Ivory Silk Crepe & 24K Gold Leaf Embroidery",
    subtitleUA: "Молочний Шовковий Креп та Золота Вишивка 24K",
    descriptionEN:
      "Modern architectural gown with a deep plunge neckline, bespoke gold metallic floral filigree embroidered along the waistline, and an elongated fluid drape with front walking slit.",
    descriptionUA:
  ...
  },
  {
    ...
    descriptionEN:
      "Sophisticated day-to-evening silhouette rendered in ultra-soft ribbed knit. High mock neck with delicate micro-button spine detailing and an effortless side walking slit.",
    descriptionUA:
    ...
    fittingTimeEN: "5-7 Business Days",
    fittingTimeUA: "5-7 Робочих Днів",
...
export default function CatalogPage() {
  ...
  const categories = [
    { id: "ALL", label: language === "UA" ? "УСІ ОБРАЗИ (7)" : "ALL PIECES (7)" },
  ...
  const filteredItems = ALL_CATALOG_ITEMS.filter(
    (item) => selectedCategory === "ALL" || item.category === selectedCategory
  );

  const activeIndex = activeItem
    ? ALL_CATALOG_ITEMS.findIndex((i) => i.id === activeItem.id)
    : -1;

  const handlePrevItem = () => {
    if (activeIndex === -1) return;
    const prevIdx = (activeIndex - 1 + ALL_CATALOG_ITEMS.length) % ALL_CATALOG_ITEMS.length;
    setActiveItem(ALL_CATALOG_ITEMS[prevIdx]);
    setFormSubmitted(false);
  };

  const handleNextItem = () => {
    if (activeIndex === -1) return;
    const nextIdx = (activeIndex + 1) % ALL_CATALOG_ITEMS.length;
    setActiveItem(ALL_CATALOG_ITEMS[nextIdx]);
    setFormSubmitted(false);
  };
  ...
  return (
    <main className="min-h-screen bg-[#060803] text-[#F3EEE6] relative selection:bg-[#C9A063] selection:text-[#060803]">
      ...
      {/* Hero Banner */}
      <section className="relative w-full min-h-[55vh] flex flex-col items-center justify-center pt-28 pb-16 px-6 bg-[#060803] border-b border-[#C9A063]/25 overflow-hidden">
        <div
          className="absolute inset-0 pointer-events-none"
          style={{
            background:
              "radial-gradient(ellipse 70% 60% at 50% 40%, rgba(201,160,99,0.08) 0%, transparent 70%, #060803 100%)",
          }}
        />
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center space-y-6 sm:space-y-8">
          <div className="flex items-center justify-center space-x-4">
            ...
            <span className="font-mono text-xs sm:text-sm text-[#C9A063] tracking-[0.35em] uppercase font-semibold">
              {language === "UA" ? "АРХІВ ТА КАТАЛОГ VÉLORA" : "VÉLORA BESPOKE ARCHIVE"}
            </span>
            <span className="h-[1px] w-12 bg-[#C9A063]/50" />
          </div>

          <h1 className="font-serif text-4xl sm:text-6xl md:text-7xl font-light text-[#F3EEE6] tracking-[0.06em] uppercase leading-[1.15] sm:leading-[1.18] lg:leading-[1.2]">
            {language === "UA" ? "Повний Каталог та Архів" : "Full Catalogue & Archive"}
          </h1>

          <p className="font-sans text-sm sm:text-base text-[#DCC8AA]/85 font-light leading-relaxed max-w-2xl mx-auto pt-2">
            {language === "UA"
              ? "Ознайомтеся з усіма 7 кутюрними образами, вечірніми силуетами та сучасним трикотажем ручної роботи. Кожна сукня створюється за індивідуальними мірками в нашому одеському ательє або приватних салонах."
              : "Discover all 7 handcrafted haute couture looks, evening silhouettes, and contemporary tailoring. Each piece is fitted individually in our Odesa atelier or private international showrooms."}
          </p>

          <div className="pt-4 flex items-center justify-center space-x-6">
            <Link
              href="/"
              className="px-6 py-2.5 border border-[#C9A063]/50 text-[#F3EEE6] hover:bg-[#C9A063] hover:text-[#060803] transition-colors font-mono text-xs uppercase tracking-widest rounded-none"
            >
              ← {language === "UA" ? "Повернутися на Головну" : "Return to Main Page"}
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 sm:px-8 md:px-12 lg:px-16 max-w-[1800px] mx-auto space-y-16 sm:space-y-20">
        ...
        {/* 7 Look Cards Grid with Alternating Directional Entrances */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 xl:gap-14">
          {filteredItems.map((item, idx) => {
            ...
            return (
              <AnimateOnScroll key={item.id} preset={slidePreset} delay={0.08 + (idx % 2) * 0.12} className="h-full">
            ...
            >
              ...
              <div className="absolute top-5 right-5 flex items-center space-x-3 z-20">
                ...
                <span className="font-mono text-xs text-[#C9A063]">
                  {activeIndex + 1}/{ALL_CATALOG_ITEMS.length}
                </span>
                <button
